...
def detect_lane_lines(frame):
    ...
    slope = None
    sit_radius = 0
    curved_closest_left_distance = float('inf')
    curved_closest_right_distance = float('inf')
    mid_line_ahri = 0
    line_center_x = 0
    closest_angle = 0
    new_x1 = 0
    ...
    left_m_abs = 0  # 기본값 초기화
    right_m_abs = 0  # 기본값 초기화
    ...
    # 기울기 값 출력 (디버깅용)
    print(f"left_m: {left_m_abs}, right_m: {right_m_abs}, good_m: {good_m}")

    
            
    
    
    new_x1, new_x2 = 0, 0  # 기본값 초기화
    ...
    if closest_line is not None:
        ...
        cv2.line(frame, (int(new_x1), int(new_y1)), (int(new_x2), int(new_y2)), (255, 105, 180), 3)
        
    # 새로운 선의 X, Y 좌표 차이 계산
    delta_x = new_x1 - new_x2
    delta_y = abs(new_y1 - new_y2)
    print(delta_x)
    # 급커브 판단 조건 (X, Y 좌표 차이 기준)
    if delta_y >= 50 and delta_x <= -18:  # 급커브 판단 기준 (y차이 50 이상, x차이 30 이상)
        sit_radius = 1
        print("오른쪽 급커브 구간입니다")
        mid_line_ahri = 1
    elif delta_y >= 50 and delta_x >= 18:  # 왼쪽 급커브 (X가 왼쪽으로 더 이동한 경우)
            sit_radius = 2
            print("왼쪽 급커브 구간입니다")
            mid_line_ahri = 1
    elif -18 < delta_x < 18:
    ...
    if sit_radius == 1: # 오른쪽 급커브
        if mid_line_ahri == 1:
            # y=337 위치에 가로선 그리기
            cv2.line(frame, (active_center_x, 0), (active_center_x, height), (0, 0, 255), 2)  # 중앙 세로선
        ...
        if abs(closest_left_distance - closest_right_distance) > 38:
            if closest_left_distance > closest_right_distance:
                ...
                sit_right = 2  # 오른쪽으로 이동
                sit_left = 0  # 왼쪽으로 이동하지 않음
            else:
                sit_left = 0  # 두 거리 차이가 동일한 경우
                sit_right = 0
                
    if sit_radius == 2: # 왼쪽 급커브
        if mid_line_ahri == 1:
            # y=337 위치에 가로선 그리기
            cv2.line(frame, (active_center_x, 0), (active_center_x, height), (255, 0, 0), 2)  # 중앙 세로선
