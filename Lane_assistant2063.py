...
def detect_lane_lines(frame):
    global sit_left, sit_right, stay, sit  # sit_left와 sit_right를 전역 변수로 사용
    ...
    if closest_left_line is not None or closest_right_line is not None:
        ...
        # start_point와 end_point를 이용하여 기울기 계산
        x1, y1 = start_point
        x2, y2 = end_point
        center_slope = (y2 - y1) / (x2 - x1) if x2 - x1 != 0 else 0
        
        if 0 < center_slope < 0.67:
            sit_left = 2
        if -0.67 < center_slope <0:
            sit_right = 2
        

        # 기울기 출력
        print(f"중앙선 기울기: {center_slope}")
        # 중앙선 그리기
...
def process_video():
    global sit_left, sit_right, stay, ddong_r, ddong_l
